...
SmartTar is a DOS 5.0 protected-mode point-of-sale / tariff management system. Current version: **2.70.1** (originally 2003, resurrected 2026).
...
st/          Application source
  ...
  util/      Build and maintenance utilities:
    ...
    ini2cfg/   Canonical source for ST.INI (app config). INI2CFG.EXE
               ...
               the CFG_DESTS rule list. ST.CFG is gitignored (generated);
               ST.INI is tracked as the single source of truth.
...
| ---- | -------------------------- |
| `st.cfg` | Generated by `util/ini2cfg/ini2cfg.exe` from `util/ini2cfg/st.ini`; MAKEFILE distributes to `bin/` and every `util/` subdir via `CFG_DESTS` |
| `st.ini` | Copied from `util/ini2cfg/st.ini` (canonical source) |
...
| `ph_info.dat` | Compiled by `util/inf2dat/inf2dat.exe` from the `.inf` sources in `util/inf2dat/` |
