...
3. **Fixed by removing `P_PDF_DIR` from struct** — Reverted field, rebuilt `ini2cfg.exe` via `mk_cfg.bat`. Excluded `P_PDF_DIR` from binary config; hardcoded `"PDF"` as the output directory in control.cpp.
