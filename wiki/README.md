...
build-docs.sh               Regenerates every output into st/docs/build/
build-docs.ps1              PowerShell Core equivalent
st/docs/build/              Generated outputs (gitignored)
```
...
```sh
./build-docs.sh        # writes .docx manuals into st/docs/build/
# or on PowerShell Core:
./build-docs.ps1
```

Requires `pandoc` on the host (this is a host-side build, not the DOS
toolchain). Outputs go to `st/docs/build/` so you can diff before promoting.
