/*= Archive Library v2.12 ================================================
 ...
 |
 +- Description ----------------------------------------------------------
 |  EX03WIN.C demonstrates ALArchiveDelete().  It lets you select a list
 |  of files to delete from an archive, then performs the deletion.  The
 |  list boxes are updated right afterwards so you can see the results.
 |
...
void ReadArchive(HWND hDlg, int archive_name_id, int list_box_id)
{
 ...
 SendDlgItemMessage(hDlg, archive_name_id, (UINT) EM_SETSEL, (WPARAM) - 1, (
    LPARAM) - 1);
...
void Delete(HWND hDlg)
{
 ...
 if (hList && hArchive) {
  ...
  EditDisplay(hDlg, AL_DEBUG, "%d file%s selected\r\n",
              count, (LPSTR) (count == 1 ? " is" : "s are"));
 /*
 ...
 */
  ...
  SetDlgItemText(hDlg, AL_OUTPUT, ALStorageGetName(ALArchiveGetStorage(hArchive)));
 ...
 */
 case WM_INITDIALOG :
  ...
  wsprintf(buf, "Windows Example 03 <instance %d>", iInstanceNumber);
