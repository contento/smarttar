...
void CONTROLLER::UpdateStatusBar(void)
{
	...
	{
		sprintf(msg, "%d errores de comunicaci�n en cabina: %s", g_cfg->MAX_COM_ERR, g_cfg->BoothInfo[RTEngine->GetComErrBooth()].Name);
	...
	{
		sprintf(msg, "%d errores de marcaci�n en cabina: %s", g_cfg->MAX_DIAL_ERR, g_cfg->BoothInfo[RTEngine->GetDialErrBooth()].Name);
		View->WStatBar->setMsg(msg, WHITE, LIGHTRED);
		RTEngine->SetDialErrBooth(-1); // reset !!!
	}
	else if (RTEngine->GetNotIncBooth() != -1 && !View->WStatBar->PendingMsg())
	{
		sprintf(msg, "Localidad no inclu�da en cabina: %s", g_cfg->BoothInfo[RTEngine->GetNotIncBooth()].Name);
		View->WStatBar->setMsg(msg, WHITE, LIGHTRED);
